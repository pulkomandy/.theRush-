...
wxString wxGraphContainer::BuildScript()
{
    ...
    if (mFinalNode)
    {
        script+= wxString::Format("return %s(", mFinalNode->GetFunctionName());
...
wxString wxGraphContainer::BuildGraphString()
{
	...
	res += itoa(mInitialState, initststr, 10);
	res += "\"\n RaknetMessage=\"";
	res += mbRaknetMessage?"1":"0";
	res += "\"\n TickHasTime=\"";
	res += mbTickHasTime?"1":"0";
	res += "\"\n Output=\"";
	...
	for (int i=0;i<mConnections.size();i++)
	{
		...
		char tmps[2048];
		sprintf(tmps,"<Connection nodesrc=\"%d\" nodedst=\"%d\" nodesrcanchor=\"%d\" nodedstanchor=\"%d\" sidesrc=\"%d\" sidedst=\"%d\" />\n",
...
*/
	if (pRootElem->Attribute("includes"))
		*GetIncludes() = XMLToCode(pRootElem->Attribute("includes"));
	else
		*GetIncludes() = "";


	if (pRootElem->Attribute("membervariables"))
		*GetMemberVariables() = XMLToCode(pRootElem->Attribute("membervariables"));
	else
		*GetMemberVariables() = "";

	if (pRootElem->Attribute("memberinit"))
		*GetMembersInit() = XMLToCode(pRootElem->Attribute("memberinit"));
	else
		*GetMembersInit() = "";


	if (pRootElem->Attribute("GraphName"))
		mGraphName = XMLToCode(pRootElem->Attribute("GraphName"));
	else
		mGraphName = "";

	if (pRootElem->Attribute("StateEnumBase"))
		mStateEnumBase = XMLToCode(pRootElem->Attribute("StateEnumBase"));
	else
		mStateEnumBase = "";

	if (pRootElem->Attribute("MessageEnumBase"))
		mMessageEnumBase = XMLToCode(pRootElem->Attribute("MessageEnumBase"));
	else
		mMessageEnumBase = "";
	...
	if (pRootElem->Attribute("Output"))
		mOutputFileName = pRootElem->Attribute("Output");
	else
		mOutputFileName = "res.h";
    ...
    while(pExtraElem)
    {
		...
		if(pNodeElem)
		{
			const char *szName = pNodeElem->Attribute("name");
			const char *szComment = pNodeElem->Attribute("comment");
			const char *szCode = pNodeElem->Attribute("code");
			...
			while(pEventElem)
			{
				const char *szEventName = pEventElem->Attribute("name");				const char *szEventCode = pEventElem->Attribute("code");

				if (!strlen(szEventName))
				{
					*pNode->GetCode() = szEventCode;
				}
				else
				{
					wxString onmachin = wxString("On ")+szEventName;
...
void wxGraphContainer::ReadString(const char *pszFileName)
{
	wxString res;
...
void wxGraphContainer::FillCodesFromNodeList(std::vector<wxString>& mList, GraphNodeType type, const char *szSubItem)
{
	std::vector<wxGraphNode*>::iterator iter = mNodes.begin();
	for (;iter != mNodes.end(); ++iter)
	{
...
wxGraphNode *wxGraphContainer::GetNodeByName(const char *szName)
{
	std::vector<wxGraphNode*>::iterator iter = mNodes.begin();
	for (;iter != mNodes.end(); ++iter)
	{
...
int wxGraphContainer::GetNodeIndexByName(const char *szName)
{
	int idx = 0;
	...
	for (; iter != mNodeCodeConnection.end(); ++iter)
	{
		...
		*/
		...
		for (int j=0;j<(*iter).second.size();j++)
		{
			...
			int AnchorSource = pNode->GetAnchorIndexByName(strsrc.c_str());
