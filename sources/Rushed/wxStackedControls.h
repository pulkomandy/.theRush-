...
public:
	...
	wxZenTextEditRoot(wxWindow* parent, wxZEditHandler * aHandler, const wxString& Lib, tstring *Str, unsigned int aID = 0)
		: wxPanel(parent, -1, wxDefaultPosition, wxDefaultSize, wxNO_BORDER)
	{
		wxSizer *mSizer = new wxBoxSizer(wxHORIZONTAL);
		mID = aID;
		mTxt = new wxTextCtrl(this, 3, _T(""), wxDefaultPosition, wxDefaultSize, wxTE_LEFT);
		mTxt->SetBackgroundColour(wxColour(197, 197, 197));
...
{
public:
	wxZenTextEdit(wxWindow* parent, wxZEditHandler * aHandler, const wxString& Lib, tstring *Str, unsigned int aID = 0)
...
public:
		...
		: wxPanel(parent, -1, wxDefaultPosition, wxDefaultSize, wxNO_BORDER)
	{
		...
		mBox = new wxComboBox(this, 2, _T(""), wxDefaultPosition, wxDefaultSize, mArray, wxCB_READONLY);
		if (mArray.GetCount())
	...
	{
		if (mValue)
			mBox->SetValue(mArray[*mValue]);
		else
			mBox->SetValue(mArray[*mValueC]);
	}
private:
...
public:
		...
		: wxPanel(parent, -1, wxDefaultPosition, wxDefaultSize, wxNO_BORDER)
	{
		...
		mBox = new wxRadioBox(this, 2, _T(""), wxDefaultPosition, wxDefaultSize, mArray, 1, wxRA_SPECIFY_COLS);
...
public:
	...
	wxZenFileOpen(wxWindow* parent, wxZEditHandler * aHandler, const wxString& Lib, const wxString& aExtensions, tstring *pFileName, bool aForOpening = true, unsigned int aID = 0)
		: wxPanel(parent, -1, wxDefaultPosition, wxDefaultSize, wxNO_BORDER)
	{
		wxSizer *mSizer = new wxBoxSizer(wxHORIZONTAL);
		mID = aID;
		...
		if (pFileName->Length())
			mBut = new wxButton(this, 2, pFileName->GetBaseName().c_str());
		else
			mBut = new wxButton(this, 2, _T("...") );
...
private:
	...
	{
		wxFileDialog fDialog(GetParent(), _T("Choose a file"), _T(""), _T(""), mExtensions,
			mbForOpening?(wxFD_OPEN|wxFD_FILE_MUST_EXIST):(wxFD_SAVE|wxFD_OVERWRITE_PROMPT));

		if (fDialog.ShowModal() == wxID_OK)
		{
			*mFileName = fDialog.GetPath().mb_str();// + fDialog.GetFilename()).mb_str();
...
public:
	...
	wxZenDirectoryOpen(wxWindow* parent, wxZEditHandler * aHandler, const wxString& Lib, tstring *pDirName, unsigned int aID = 0)
		: wxPanel(parent, -1, wxDefaultPosition, wxDefaultSize, wxNO_BORDER)
	{
		wxSizer *mSizer = new wxBoxSizer(wxHORIZONTAL);
		mID = aID;
		...
		if (pDirName->Length())
			mBut = new wxButton(this, 2, pDirName->c_str());
		else
			mBut = new wxButton(this, 2, _T("...") );
...
public:
	...
	wxZenGradient(wxWindow* parent, wxZEditHandler * aHandler,
		...
		float *aLifeTime = NULL, unsigned int aID = 0)
		: wxPanel(parent, -1, wxDefaultPosition, wxDefaultSize, wxNO_BORDER)
	{
		mID = aID;
		wxSizer *mSizer = new wxBoxSizer(wxHORIZONTAL);
...
public:
		...
		: wxPanel(parent, -1, wxDefaultPosition, wxDefaultSize, wxNO_BORDER)
	{
		...
		mRootID = mTV->AddRoot(_T(""));
	}

	~wxZenTreeView() {}

	virtual wxTreeItemId AddItem(const char *szStr)
	{
	...
	virtual wxTreeItemId AddItem(const char *szStr, wxTreeItemId rt)
	{
