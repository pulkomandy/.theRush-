...
bool CDDSImage::load(unsigned char *aStream, bool flipImage)
{
	...
	//TGEStreamRead(aStream, sizeof(DDS_HEADER), &ddsh);
	memcpy(&ddsh, aStream, sizeof(DDS_HEADER));
	aStream += sizeof(DDS_HEADER);
    ...
    for (unsigned int n = 0; n < (unsigned int)(m_type == TextureCubemap ? 6 : 1); n++)
    {
		...
		//TGEStreamRead(aStream, size, pixels);
		memcpy(pixels, aStream, size);
		aStream += size;
...
GLuint UploadDDS(unsigned char *tgeStream, int &w, int &h, int& nbmml)
{
	...
	if (img.is_cubemap())
	{
		glBindTexture(GL_TEXTURE_CUBE_MAP_EXT, texobj);
