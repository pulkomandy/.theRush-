...
#define FXPLAYER_FILENAME   "../Resources/RoomsBrowserGFx3.swf"
...
{    
    ...
    if (!(pMovie = *pMovieDef->CreateInstance(false)))
        return 1;
	...
	pMovie->SetLog(&outputToConsoleLog);
	...
	sampleImpl.Init(&lobby2Client, &messageFactory, rakPeer, &roomsPlugin, "Test Title Name", "Test secret key", "properties.xml", lanServerPort, pDelegate, pMovie);
	...
	// Unregister custom logger to prevent crash on shutdown
	pMovie->SetLog(0);
