...
namespace MiniBall
{
	...
	Vector Matrix::operator*(const Vector &v) const
	{
		...
		return Vector(M(1, 1) * v.getX() + M(1, 2) * v.getY() + M(1, 3) * v.getZ(),
		              M(2, 1) * v.getX() + M(2, 2) * v.getY() + M(2, 3) * v.getZ(),
		              M(3, 1) * v.getX() + M(3, 2) * v.getY() + M(3, 3) * v.getZ());
	}
	...
	Point Matrix::operator*(const Point &P) const
	{
		...
		return Point(M(1, 1) * P.getX() + M(1, 2) * P.getY() + M(1, 3) * P.getZ() + M(1, 4),
		             M(2, 1) * P.getX() + M(2, 2) * P.getY() + M(2, 3) * P.getZ() + M(2, 4),
		             M(3, 1) * P.getX() + M(3, 2) * P.getY() + M(3, 3) * P.getZ() + M(3, 4));
	}
	...
	Quaternion Matrix::operator*(const Quaternion &Q) const
	{
		...
		return Quaternion(M(1, 1) * Q.getX() + M(1, 2) * Q.getY() + M(1, 3) * Q.getZ() + M(1, 4) * Q.getW(),
		                  M(2, 1) * Q.getX() + M(2, 2) * Q.getY() + M(2, 3) * Q.getZ() + M(2, 4) * Q.getW(),
		                  M(3, 1) * Q.getX() + M(3, 2) * Q.getY() + M(3, 3) * Q.getZ() + M(3, 4) * Q.getW(),
		                  M(4, 1) * Q.getX() + M(4, 2) * Q.getY() + M(4, 3) * Q.getZ() + M(4, 4) * Q.getW());
	}

	Vector operator*(const Vector &v, const Matrix &M)
	{
		return Vector(v.getX() * M(1, 1) + v.getY() * M(2, 1) + v.getZ() * M(3, 1) + M(4, 1),
		              v.getX() * M(1, 2) + v.getY() * M(2, 2) + v.getZ() * M(3, 2) + M(4, 2),
		              v.getX() * M(1, 3) + v.getY() * M(2, 3) + v.getZ() * M(3, 3) + M(4, 3));
	}

	Point operator*(const Point &P, const Matrix &M)
	{
		return Point(P.getX() * M(1, 1) + P.getY() * M(2, 1) + P.getZ() * M(3, 1),
		             P.getX() * M(1, 2) + P.getY() * M(2, 2) + P.getZ() * M(3, 2),
		             P.getX() * M(1, 3) + P.getY() * M(2, 3) + P.getZ() * M(3, 3));
	}

	Quaternion operator*(const Quaternion &Q, const Matrix &M)
	{
		return Quaternion(Q.getX() * M(1, 1) + Q.getY() * M(2, 1) + Q.getZ() + M(3, 1) + Q.getW() * M(4, 1),
		                  Q.getX() * M(1, 2) + Q.getY() * M(2, 2) + Q.getZ() + M(3, 2) + Q.getW() * M(4, 2),
		                  Q.getX() * M(1, 3) + Q.getY() * M(2, 3) + Q.getZ() + M(3, 3) + Q.getW() * M(4, 3),
		                  Q.getX() * M(1, 4) + Q.getY() * M(2, 4) + Q.getZ() + M(3, 4) + Q.getW() * M(4, 4));
	}
...
/*
	Matrix &Matrix::orthogonalise()
	{
		...
		M(1, 1) = v1.getX();  M(1, 2) = v2.getX();  M(1, 3) = v3.getX();
		M(2, 1) = v1.getY();  M(2, 2) = v2.getY();  M(2, 3) = v3.getY();
		M(3, 1) = v1.getZ();  M(3, 2) = v2.getZ();  M(3, 3) = v3.getZ();
	...
	{
		return eulerRotate(v.getX(), v.getY(), v.getZ());
	}
